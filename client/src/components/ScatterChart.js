    ...
    React.useEffect(() => {
      ...
      const data = () => {
        getUsers().then((res) => {
          ...
          res.forEach((user) => {
            if (user['is_fake'] === 0){
              true_follower_following.push({x: user['user_follower_count'], y: user['user_following_count'], id : user['id']})}
            else
              false_follower_following.push({x: user['user_follower_count'], y: user['user_following_count'], id : user['id']})
          });
          setDataset([
            {
              label: "Real account",
              ...
              pointBackgroundColor: function(context) {
                  let color = 'yellow'
                  if (!(context.dataset.data[context.dataIndex].id == idUser.idUser))
